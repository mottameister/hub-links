...
const searchPaymentsByExternalReference = async (externalReference) => {
  ...
  const payload = await mercadoPagoFetch(`/v1/payments/search?${query.toString()}`);
  return (payload.results || []).filter((payment) => (
    payment.external_reference === externalReference
    || payment.metadata?.order_id === externalReference
  ));
};
