...
const blobOptions = (access = "private") => ({
  access,
  token: process.env.BLOB_READ_WRITE_TOKEN,
});

const isForbiddenBlobError = (error) => String(error && error.message || "").includes("403 Forbidden");
...
const readRegistrations = async (eventId) => {
  ...
  try {
    let blob;
    try {
      blob = await get(blobPath(eventId), blobOptions("private"));
    } catch (error) {
      if (!isForbiddenBlobError(error)) throw error;
      blob = await get(blobPath(eventId), blobOptions("public"));
    }
